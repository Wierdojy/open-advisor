...
function renderResearch() {
  const { holdings, watchlistAssets } = getDashboardData();
  const assetPool = [...holdings, ...watchlistAssets].map(({ asset }, index) => ({
    asset,
    performance: performanceForAsset(asset, index + 1)
  }));
  ...
  const cards = (assetPool.length ? assetPool : fallbackAssets.map((asset, index) => ({ asset, performance: performanceForAsset(asset, index + 1) }))).slice(0, 3);
  const researchSearch = uiState.researchQuery || '';

  el('view-research').innerHTML = `
    <div class="tab-scene tab-scene--research stitch-research-scene">
      ${renderViewAtmosphere('research', 24)}
      <div class="stitch-research-hero">
        <h2 class="stitch-research-title">Research</h2>
        <div class="stitch-research-subtitle">Explore assets &amp; market data</div>
      </div>
      <div class="stitch-research-search-shell">
        <form id="research-search-form" class="stitch-research-search" role="search">
          <span class="material-symbols-outlined">search</span>
          <input class="stitch-research-input" name="query" value="${researchSearch}" placeholder="Search stocks, crypto, or indices" />
          <button class="stitch-research-mic" type="submit" aria-label="Search">
            <span class="material-symbols-outlined">mic</span>
          </button>
        </form>
      </div>
      <div class="stitch-research-tabs" aria-label="Research modes">
        <button class="stitch-research-tab stitch-research-tab--active" type="button">Trending</button>
        <button class="stitch-research-tab" type="button">Analysis</button>
      </div>
      <div class="stitch-research-list">
        ${cards.map(({ asset, performance }, index) => `
          <article class="stitch-asset-card">
            <div class="stitch-asset-card__icon stitch-asset-card__icon--${index}">
              <div class="stitch-asset-card__glyph">${(asset.symbol || asset.name || '?').slice(0, 1)}</div>
            </div>
            <div class="stitch-asset-card__copy">
              <div class="stitch-asset-card__symbol">${asset.symbol || asset.name}</div>
              <div class="stitch-asset-card__name">${(asset.name || asset.symbol || 'Tracked asset').toUpperCase()}</div>
            </div>
            <div class="stitch-asset-card__price">
              <div class="stitch-asset-card__value">${formatCurrency(performance.price)}</div>
              <div class="stitch-asset-card__change ${performance.positive ? 'positive' : 'negative'}">${performance.positive ? '+' : ''}${performance.change}%</div>
            </div>
          </article>
        `).join('')}
      </div>
